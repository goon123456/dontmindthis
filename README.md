...
The Player tab includes rebindable noclip, vehicle fly, hitbox extending for players/NPCs, and a cleaner ESP panel with enable/keybind controls, max distance, chams, name/distance/health display toggles, and category toggles for players, zombies, vehicles, and corpses. Non-player ESP categories use budgeted scans and start off by default to reduce lag. Vehicle fly uses WASD with Space/E to rise and LeftCtrl/Q to descend while seated in a vehicle.
